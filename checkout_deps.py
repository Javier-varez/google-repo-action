...
def checkout_root_project(project_root, root_project, branch):
    ...
    run_command(f"git fetch {root_project['remote']['name']} {branch}",
                directory)
    run_command("git reset --hard FETCH_HEAD", directory)
...
def download_dependencies(projects, dependencies, branch, project_root):
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Downloads changes in the associated repositories')
    parser.add_argument('branch', help='branch to sync across projects')
    parser.add_argument('root_project',
                        help='name of the root project that contains the' +
                        'dependency list')
    ...
    if root_project is None:
        raise Exception(f'Root project {args.root_project} not found!')
